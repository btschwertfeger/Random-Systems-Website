...
const linstocheq_A_slide = document.getElementById("linstocheq_A_slide"),
  ...
  linstocheq_value_fields = document.getElementsByName(
    "linstocheq_slide_value",
  ),
...
linstocheq_RESET_BTN.onclick = function () {
  ...
  linstocheq_SLIDER.forEach((element, index) => {
    // reset the sliders
    const default_value =
      window.linstocheq_default_input[linstocheq_plot_variables[index]];
    document.getElementById(element.id).value = default_value;
  });
  linstocheq_value_fields.forEach((element, index) => {
    // Reset value fields
    const default_value =
      window.linstocheq_default_input[linstocheq_plot_variables[index]];
...
const linstocheq_PLOT_AGAIN_BTN = document.getElementById(
  "linstocheq_plotAgainBtn",
);
...
for (let entry = 0; entry < linstocheq_SLIDER.length; entry++) {
  linstocheq_SLIDER[entry].oninput = function () {
    ...
    document.getElementById(elem_id + "value").innerHTML =
      document.getElementById(linstocheq_SLIDER[entry].id).value;
  };
...
const random_walks_n_slide = document.getElementById("random_walks_n_slide"),
  random_walks_nstep_slide = document.getElementById(
    "random_walks_nsteps_slide",
  ),
  random_walks_SLIDER = document.getElementsByName("1drandomslider"),
  random_walks_value_fields = document.getElementsByName(
    "random_walks_slide_value",
  ),
...
random_walks_RESET_BTN.onclick = function () {
  ...
  random_walks_SLIDER.forEach((element, index) => {
    // reset the sliders
    const default_value =
      window.random_walks_defaultInput[random_walks_plot_variables[index]];
    document.getElementById(element.id).value = default_value;
  });
  random_walks_value_fields.forEach((element, index) => {
    // Reset value fields
    const default_value =
      window.random_walks_defaultInput[random_walks_plot_variables[index]];
    document.getElementById(element.id).innerHTML = default_value;
...
const random_walks_PLOT_AGAIN_BTN = document.getElementById(
  "random_walks_plotAgainBtn",
);
...
for (let entry = 0; entry < random_walks_SLIDER.length; entry++) {
  random_walks_SLIDER[entry].oninput = function () {
    ...
    elem_id = elem_id.substring(0, elem_id.length - 5);
    document.getElementById(elem_id + "value").innerHTML =
      document.getElementById(random_walks_SLIDER[entry].id).value;
  };
